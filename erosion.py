"""
THIS CODE IS TAKEN FROM WORLDENGINE (https://github.com/Mindwerks/worldengine)
It was slightly adapted for the purpose of the article.
"""
...
class ErosionSimulation(object):
    ...
    def find_quick_path(self, river, world):
        ...
        x, y = river
        new_path = []
        lowest_elevation = world.elevation[y, x]
        # lowestDirection = [0, 0]
        ...
        for dx, dy in DIR_NEIGHBORS:
            ...
            elevation = world.elevation[ty, tx]
    ...
    def river_sources(world, water_flow, water_path):
        ...
        for x in range(0, world.width - 1):
            for y in range(0, world.height - 1):
                rain_fall = world.precipitations[y, x]
                ...
                while not neighbour_seed_found:
                    ...
                    if world.is_mountain((cx, cy)) and water_flow[cy, cx] >= RIVER_TH:
                        ...
                        for seed in river_source_list:
                            sx, sy = seed
                            if in_circle(DISTANCE_BETWEEN_SOURCES, cx, cy, sx, sy):
        ...
        while True:
            ...
            if lower_elevation and not is_wrapped:
                lower_path = astar.PathFinder().find(
                    world.elevation, current_location, lower_elevation)
                if lower_path:
                    path += lower_path
    ...
    def findLowerElevation(self, source, world):
        ...
        currentRadius = 1
        maxRadius = 40
        lowestElevation = world.elevation[y, x]
        destination = []
        notFound = True
        isWrapped = False
        wrapped = []
        ...
        while notFound and currentRadius <= maxRadius:
            for cx in range(-currentRadius, currentRadius + 1):
                for cy in range(-currentRadius, currentRadius + 1):
                    ...
                    elevation = world.elevation[ry, rx]
            ...
            for x in range(rx - radius, rx + radius):
                for y in range(ry - radius, ry + radius):
                    ...
                    if [x, y] in river:  # ignore river itself
                        continue
                    if world.elevation[y, x] <= world.elevation[ry, rx]:
                    ...
                    diff = world.elevation[ry, rx] - world.elevation[y, x]
                    newElevation = world.elevation[y, x] + (
                        diff * curve)
                    if newElevation <= world.elevation[ry, rx]:
                        print('newElevation is <= than river, fix me...')
                        newElevation = world.elevation[ry, rx]
                    world.elevation[y, x] = newElevation
        return
